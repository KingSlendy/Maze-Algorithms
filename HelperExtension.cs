...
namespace Maze_Algorithms {
    static class HelperExtension {
        ...
        public static T[] Fill<T>(this T[] source, int start, int end, T value) {
            ...
            if (end > source.Length || end < start) throw new ArgumentOutOfRangeException("The end variable has invalid value.");
        ...
        public static T[,] ToArray2D<T>(this T[] array1D, int rowLen, int colLen) {
            ...
            for (var i = 0; i < (rowLen * colLen); i++)
                setArrayTD[i / colLen, i % colLen] = (i < array1D.Length) ? array1D[i] : default;
        ...
        /// <summary>
        /// Returns the default of the given type if the condition is not true, else returns the element.
        /// </summary>
        public static T ThenDefault<T>(this bool condition, T element) {
            return (condition) ? element : default;
        }

        /// <summary>
        /// Converts RGB color to HSV color.
        /// </summary>
        public static Color ToHSV(this Color color) {
            var max = Math.Max(color.R, Math.Max(color.G, color.B));
            var min = Math.Min(color.R, Math.Min(color.G, color.B));
            var hue = color.GetHue();
            var saturation = (max == 0) ? 0 : 1d - (1d * min / max);
            var value = max / 255d;
            return color.ToHSV(hue, saturation, value);
        }

        /// <summary>
        /// Returns a new HSV color.
        /// </summary>
        public static Color ToHSV(this Color color, double hue, double saturation, double value) {
            var index = (int)Math.Floor(hue / 60) % 6;
